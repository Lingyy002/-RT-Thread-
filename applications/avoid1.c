...
typedef enum { MOVE_STOP = 0, MOVE_FORWARD, MOVE_LEFT, MOVE_RIGHT } MoveState;
...
static void control_thread(void *parameter) {
    while (1) {
        ...
        // 获取当前yaw角用于方向修正（只在MOVE_FORWARD时启用）
        int yaw_correction = 0;
        static float smooth_yaw = 0;  // 一阶滤波变量

        if (current_move_state == MOVE_FORWARD) {
            float raw_yaw = mpu6050_get_yaw();  // 单位：度
            smooth_yaw = 0.9f * smooth_yaw + 0.1f * raw_yaw;

            if (smooth_yaw > -1.0f && smooth_yaw < 1.0f) {
                yaw_correction = 0;  // 死区 ±1°
            } else {
                yaw_correction = (int)(smooth_yaw * 10);  // 降低修正比例
            }
        ...
        for (int i = 0; i < MOTOR_COUNT; i++) {
            ...
            // 在PWM阶段加入角度修正（左右电机）
            if (current_move_state == MOVE_FORWARD) {
                if (i == 0 || i == 2) pwm += yaw_correction;  // 左轮加修正
                if (i == 1 || i == 3) pwm -= yaw_correction;  // 右轮减修正
            }
...
static void obstacle_avoidance_thread(void *param) {
    ...
    while (1) {
        for (int i = 0; i < DIRECTION_COUNT; i++) directions[i].min_dist = 10000.0f;

        for (int i = 0; i < 1000; i++) {
            rplidar_response_measurement_node_t node;
            if (rp_lidar_get_scan_data(lidar, &node, 1000) != RT_EOK) continue;
...
static void mpu_test_thread(void *param)
{
    rt_kprintf("[MPU TEST] Starting MPU yaw monitor...\n");

    while (1)
    {
        mpu6050_update_yaw();
        float yaw_deg = mpu6050_get_yaw();
        rt_kprintf("[MPU TEST] Yaw = %d deg\n", (int)yaw_deg);
        rt_thread_mdelay(15);
    }
}
...
int mpu_test(int argc, char *argv[])
{
    if (mpu_test_tid == RT_NULL)
    {
        ...
        mpu_test_tid = rt_thread_create("mpu_test",
                                        mpu_test_thread,
                                        RT_NULL,
                                        2048, 20, 10);
        if (mpu_test_tid)
        {
            rt_thread_startup(mpu_test_tid);
            return 0;
        }
        else
        {
            rt_kprintf("[MPU TEST] Failed to start thread\n");
            return -1;
        }
    }

    rt_kprintf("[MPU TEST] Already running\n");
    return 0;
}
MSH_CMD_EXPORT(mpu_test, Test MPU6050 yaw output);
